...
const MyLikesPage = () => {
  ...
  useEffect(() => {
    const fetchLikes = async () => {
      ...
      const likedIds = ratingsSnap.docs
        .filter(doc => doc.data().fromUserId === currentUserId && doc.data().rating >= 4)
        .map(doc => doc.data().toUserId)
        .filter(uid => !blocked.includes(uid));

      const likedSnaps = await Promise.all(likedIds.map(uid => getDoc(doc(db, 'users', uid))));
      const users = likedSnaps
  ...
  return (
    <div style={styles.wrapper}>
      <h2 style={styles.heading}>People You Liked</h2>
      {likes.length === 0 ? (
        <p style={styles.meta}>You haven’t liked anyone yet.</p>
      ) : (
        likes.map(user => (
